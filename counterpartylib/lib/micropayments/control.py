...
from jsonrpc import dispatcher ##added by chris
...
def get_deposit_expire_time(script, netcode):
    return scripts.get_deposit_expire_time(script) 

def get_script_address(script, netcode):
    return util.script_address(script, netcode)
...
def get_deposit_payee_pubkey(script):
    return scripts.get_deposit_payee_pubkey(script)

def get_commit_payee_pubkey(script):
    return scripts.get_commit_payee_pubkey(script)

def get_commit_spend_secret_hash(script):
    return scripts.get_commit_spend_secret_hash(script)

def get_deposit_payer_pubkey(script):
    return scripts.get_deposit_payer_pubkey(script)

def get_txs_func(txids):
    return dispatcher.get("getrawtransaction_batch")(txhash_list=txids)

def sign_deposit(rawtxs,wif,unsigned_rawtx):
    return scripts.sign_deposit(get_txs_func,wif,unsigned_rawtx)

def sign_payout_recover(rawtxs,wif,payout_rawtx,commit_script,spend_secret):
    return scripts.sign_payout_recover(get_txs_func,wif,payout_rawtx,commit_script,spend_secret)
       
def sign_revoke_recover(rawtxs,wif,revoke_rawtx,commit_script,revoke_secret):
    return scripts.sign_revoke_recover(get_txs_func,wif,revoke_rawtx,commit_script,revoke_secret)
 
def sign_change_recover(rawtxs,wif,change_rawtx,deposit_script,spend_secret):
    return scripts.sign_change_recover(get_txs_func,wif,change_rawtx,deposit_script,spend_secret)
   
def sign_expire_recover(rawtxs,wif,expire_rawtx,deposit_script):
    return scripts.sign_expire_recover(get_txs_func,wif,expire_rawtx,deposit_script)

def sign_finalize_commit(rawtxs,wif,commit_rawtx,deposit_script):
    return scripts.sign_finalize_commit(get_txs_func,wif,commit_rawtx,deposit_script)

def sign_created_commit(rawtxs, wif, unsigned_rawtx, deposit_script_hex):
    return scripts.sign_created_commit(get_txs_func, wif, unsigned_rawtx, deposit_script_hex)

def netcode_from_wif(wif):
    return keys.netcode_from_wif(wif)
